...
void	error_manager(t_env env, enum e_error signal)
{
	...
	if (signal == INVALID_NB_OF_CYCLES)
	...
	else if (signal == READ_FILE_FAILED)
		ft_putendl_fd("occurred while attempting to read a champion's program.", STDERR_FILENO);
		...
		ft_putendl_fd("please make sure the files are Corewar champions.", STDERR_FILENO);
	else
		perror(NULL);
	ft_putendl(VM_USAGE);
	exit(EXIT_FAILURE);
