...
/*   Updated: 2018/03/28 14:04:59 by gudemare         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					do_and(t_process *process, t_env *env)
{
	...
	skip = 2;
	type[0] = get_param_type(env, process->pc, OP_AND, 0);
	type[1] = get_param_type(env, process->pc, OP_AND, 1);
	type[2] = get_param_type(env, process->pc, OP_AND, 2);
	if (!(type[0] != 0 && type[1] != 0 && type[2] == T_REG))
		return (0);
	value[0] = get_param_raw_value(env, process->pc + skip, type[0], OP_AND);
	skip += get_skip(type[0]);
	value[1] = get_param_raw_value(env, process->pc + skip, type[1], OP_AND);
	skip += get_skip(type[1]);
	value[2] = get_param_raw_value(env, process->pc + skip, type[2], OP_AND);
	if (!(is_reg(value[2])
