...
/*   Updated: 2018/03/23 18:41:27 by gudemare         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			disp_process_state(t_env *env, t_process *proc)
{
	...
	if (proc->cycle_to_wait != 0)
		ft_printf("\x1b[KLe process du joueur %s%-4d\x1b[0m dort \
durant \x1b[32m%-4d\x1b[0m cycles au pc \x1b[1;36m%-4d\x1b[0m ",
		...
		ft_printf("\x1b[KLe process du joueur %s%-4d\x1b[0m \
effectue un \x1b[1;32m%-4s (%#-.2hx)\x1b[0m au pc \x1b[1;36m%-4d\x1b[0m ",
	...
	while (i < REG_NUMBER)
	{
		ft_printf("%s%x\x1b[0m%c", (proc->reg[i] == 0) ? "\x1b[2m" : "\x1b[0m",
				proc->reg[i], (i == 15) ? '}' : ',');
		i++;
	}
	ft_putstr("\n\x1b[K");
